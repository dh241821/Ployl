...
class User:
    ...
    def _normalize_location_id(self, standort_id: Optional[int]) -> Optional[int]:
        if standort_id is None:
            return None
        try:
            return int(standort_id)
    ...
    def can_read_location(self, standort_id: Optional[int]) -> bool:
        if not self.location_permissions:
            return True
        normalized = self._normalize_location_id(standort_id)
        if normalized is None:
            return True
        permission = self.location_permissions.get(normalized)
