...
class DatabaseManager:
    ...
    def __init__(self, db_path: Optional[Path] = None) -> None:
        ...
        self._vehicle_cache: Dict[int, sqlite3.Row] = {}
        self._category_name_cache: Dict[int, str] = {}
        self._product_type_cache: Dict[int, str] = {}
        self._product_model_cache: Dict[int, str] = {}
        self._manufacturer_name_cache: Dict[int, str] = {}
    ...
    def _vehicle_label(self, fahrzeug_id: Optional[int]) -> str:
        if not fahrzeug_id:
            return ""
        try:
            vehicle_id = int(fahrzeug_id)
        ...
        if vehicle_id not in self._vehicle_cache:
            ...
            if not row:
                return f"Fahrzeug #{vehicle_id}"
            self._vehicle_cache[vehicle_id] = row
        cached = self._vehicle_cache[vehicle_id]
        label = cached["name"] or f"Fahrzeug #{vehicle_id}"
    ...
    def _lookup_cached_label(
        ...
        try:
            ...
            return ""
        if key not in cache:
            row = self.connection.execute(query, (key,)).fetchone()
            if row and row[0]:
                cache[key] = str(row[0])
            else:
                cache[key] = f"{fallback} #{key}"
        return cache[key]

    def _category_label(self, category_id: Optional[int]) -> str:
        return self._lookup_cached_label(
            self._category_name_cache,
            "SELECT name FROM kategorien WHERE id = ?",
            category_id,
            "Kategorie",
        )

    def _product_type_label(self, typ_id: Optional[int]) -> str:
        return self._lookup_cached_label(
    ...
    def _product_model_label(self, model_id: Optional[int]) -> str:
        return self._lookup_cached_label(
            self._product_model_cache,
            "SELECT name FROM produkt_modelle WHERE id = ?",
            model_id,
            "Produktmodell",
        )

    def _product_manufacturer_label(self, manufacturer_id: Optional[int]) -> str:
        return self._lookup_cached_label(
            self._manufacturer_name_cache,
            "SELECT name FROM produkt_hersteller WHERE id = ?",
            manufacturer_id,
            "Hersteller",
        )
    ...
    def add_product_type(self, name: str) -> int:
        with self.connection:
            ...
            new_id = int(cur.lastrowid)
        self._product_type_cache.clear()
        return new_id
    ...
    def add_product_model(self, typ_id: int, name: str) -> int:
        with self.connection:
            ...
            new_id = int(cur.lastrowid)
        self._product_model_cache.clear()
        return new_id
    ...
    def add_product_manufacturer(self, name: str) -> int:
        with self.connection:
            ...
            new_id = int(cur.lastrowid)
        self._manufacturer_name_cache.clear()
        return new_id
        ...
        *,
        ...
        if produkt_id:
            existing_row = self.connection.execute(
                """
                SELECT name, typ, seriennummer, hersteller, anschaffungsdatum, kategorie_id,
                       standort_id, fahrzeug_id, status, interne_kennung, stk_intervall,
                       mtk_intervall, letzte_stk, letzte_mtk, naechste_stk, naechste_mtk,
                       stk_aktiv, mtk_aktiv, lagerort, produkt_typ_id, produkt_modell_id,
                       produkt_hersteller_id, informationstext
                FROM produkte
                WHERE id = ? AND mandant_id = ?
                """,
                (produkt_id, self._active_mandant_id),
            ...
            if existing_row:
                previous_vehicle_id = existing_row["fahrzeug_id"]
        payload = {
        ...
        *,
        ...
        with self.connection:
            if produkt_id:
                ...
                change_summary = self._summarize_product_changes(
                    existing_row,
                    {
                        "name": name,
                        "typ": typ,
                        "seriennummer": seriennummer,
                        "hersteller": hersteller,
                        "anschaffungsdatum": anschaffungsdatum_str,
                        "kategorie_id": kategorie_id,
                        "standort_id": standort_id,
                        "fahrzeug_id": fahrzeug_id,
                        "status": status,
                        ...
                        "stk_intervall": stk_intervall,
                        "mtk_intervall": mtk_intervall,
                        "letzte_stk": letzte_stk_str,
                        "letzte_mtk": letzte_mtk_str,
                        "naechste_stk": naechste_stk_str,
                        "naechste_mtk": naechste_mtk_str,
                        "stk_aktiv": stk_flag,
                        "mtk_aktiv": mtk_flag,
                        "lagerort": lagerort,
                        "produkt_typ_id": produkt_typ_id,
            ...
            values_with_mandant = values + (self._active_mandant_id,)
            placeholders = ", ".join(["?"] * len(values_with_mandant))
            cur = self.connection.execute(
                f"""
                INSERT INTO produkte (
                    ...
                    produkt_modell_id, produkt_hersteller_id, informationstext, mandant_id
                )
                VALUES ({placeholders})
                """,
                values_with_mandant,
    ...
    def _summarize_product_changes(
        self,
        previous: Optional[sqlite3.Row],
        new_values: Dict[str, Any],
    ) -> str:
        if not previous:
            return "Produktdaten aktualisiert"

        def display_bool(value: Any) -> str:
            return "Aktiv" if bool(value) else "Inaktiv"

        def display_interval(value: Any) -> str:
            if value is None:
                return ""
            try:
                months = int(value)
        ...
        field_specs: List[Tuple[str, str, Any]] = [
            ...
            ("typ", "Typ/Modell", lambda v: v or ""),
            ("seriennummer", "Seriennummer", lambda v: v or ""),
            ("hersteller", "Hersteller", lambda v: v or ""),
            ("anschaffungsdatum", "Anschaffungsdatum", self._display_date),
            ("kategorie_id", "Kategorie", self._category_label),
            ("standort_id", "Standort", self.location_label),
            ("fahrzeug_id", "Fahrzeug", self._vehicle_label),
            ("status", "Status", lambda v: STATUS_LABELS.get(v, v or "")),
            ("interne_kennung", "Interne Kennung", lambda v: v or ""),
            ("stk_intervall", "STK-Intervall", display_interval),
            ("mtk_intervall", "MTK-Intervall", display_interval),
            ("letzte_stk", "Letzte STK", self._display_date),
            ("letzte_mtk", "Letzte MTK", self._display_date),
            ("naechste_stk", "Nächste STK", self._display_date),
            ("naechste_mtk", "Nächste MTK", self._display_date),
            ("stk_aktiv", "STK aktiv", display_bool),
            ("mtk_aktiv", "MTK aktiv", display_bool),
            ("lagerort", "Lagerort", lambda v: v or ""),
            ("produkt_typ_id", "Produkttyp", self._product_type_label),
            ("produkt_modell_id", "Produktmodell", self._product_model_label),
            ("produkt_hersteller_id", "Produkt-Hersteller", self._product_manufacturer_label),
            ("informationstext", "Information", lambda v: v or ""),
        ]

        changes: List[str] = []
        for column, label, formatter in field_specs:
            old_value = previous[column] if column in previous.keys() else None
            new_value = new_values.get(column)
            compare_old = old_value
            compare_new = new_value
            if column in {"standort_id", "fahrzeug_id"}:
                try:
                    compare_old = int(old_value) if old_value is not None else None
                except (TypeError, ValueError):
                    compare_old = None
                try:
                    compare_new = int(new_value) if new_value is not None else None
                except (TypeError, ValueError):
                    compare_new = None
            elif column in {"stk_intervall", "mtk_intervall"}:
                try:
                    compare_old = int(old_value) if old_value is not None else None
                except (TypeError, ValueError):
                    compare_old = old_value
                try:
                    ...
                    compare_new = new_value
            elif column in {"stk_aktiv", "mtk_aktiv"}:
                compare_old = bool(old_value)
                compare_new = bool(new_value)
            if compare_old == compare_new:
                continue
            try:
                old_display = formatter(old_value)
            except Exception:
            ...
            if old_display == new_display:
                continue
            old_text = old_display if old_display not in (None, "") else "—"
            new_text = new_display if new_display not in (None, "") else "—"
            changes.append(f"{label}: {old_text} → {new_text}")
        if not changes:
            return "Produktdaten aktualisiert"
        return "Aktualisiert – " + "; ".join(changes)
