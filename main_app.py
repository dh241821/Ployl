...
class ProductsView(ttkb.Frame):
    ...
    def refresh(self) -> None:
        ...
        for row in rows:
            raw_location = row["standort_id"] if "standort_id" in row.keys() else None
            standort_id = self._as_int(raw_location)
        ...
        for row in accessible_rows:
            ...
            standort_id = self._as_int(row["standort_id"] if "standort_id" in row.keys() else None)
            ...
            fahrzeug_id = self._as_int(row["fahrzeug_id"] if "fahrzeug_id" in row.keys() else None)
    ...
    def _update_filter_values(self, rows: List[sqlite3.Row]) -> None:
        ...
        for row in rows:
            ...
            if standort_id is None:
                ...
                location_map[label] = standort_id
            ...
            if fahrzeug_id is None:
                ...
                vehicle_map[vehicle_label] = fahrzeug_id
...
class ComponentSearchView(ttkb.Frame):
    ...
    def export_lifecycle(self) -> None:
        ...
        component_id, row = selection
        try:
            html = self.db.component_lifecycle_report(component_id)
        except Exception as exc:  # pragma: no cover - UI safeguard
            Messagebox.show_error(str(exc), "Fehler")
            return
        prefix = slugify_filename(
            f"komponente_{row['name']}_{row['seriennummer'] or component_id}"
        )
        path = build_report_path(prefix, "html")
        path.write_text(html, encoding="utf-8")
        notify_report_saved(path, "Komponenten-Lebenslauf")
...
class VehiclesView(ttkb.Frame):
    def __init__(self, master: tk.Misc, db: DatabaseManager) -> None:
        ...
        self.location_filter = ttkb.StringVar(value="Alle")
    ...
    def _ensure_write_for_vehicle(self, fahrzeug_id: Optional[int]) -> bool:
        if not self.user or not self.user.location_permissions or fahrzeug_id is None:
            return True
        vehicle = self.db.get_vehicle(fahrzeug_id)
        if not vehicle:
            Messagebox.show_error("Fahrzeug nicht gefunden", "Fehler")
            return False
        standort_id = vehicle["standort_id"] if "standort_id" in vehicle.keys() else None
        if self.user.can_write_location(standort_id):
            return True
        Messagebox.show_info(
            "Sie haben keine Schreibrechte für den Standort dieses Fahrzeugs.",
            "Keine Berechtigung",
        )
        return False
    ...
    def refresh(self) -> None:
        ...
        self._update_filter_options(accessible_rows)

        self.table.delete_rows()
        location_filter_label = self.location_filter.get()
        location_filter_id = self._location_filter_map.get(location_filter_label)
        ...
        for row in accessible_rows:
            ...
            if location_filter_id is not None:
                if location_filter_id == -1:
                    if standort_id is not None:
                        continue
                elif standort_id != location_filter_id:
                    continue
    ...
    def edit_vehicle(self) -> None:
        ...
        if not vehicle_id:
            return
        if not self._ensure_write_for_vehicle(vehicle_id):
            return
    ...
    def _update_filter_options(self, rows: List[sqlite3.Row]) -> None:
        location_map: Dict[str, Optional[int]] = {"Alle": None}
        ...
        for row in rows:
            ...
            if standort_id is None:
                has_unassigned = True
                continue
            label = self.db.location_label(int(standort_id)) or f"Standort #{standort_id}"
            if label in location_map:
                label = f"{label} (ID {standort_id})"
            location_map[label] = standort_id
        if has_unassigned:
        ...
        self._location_filter_map = location_map
        self.location_box.configure(values=list(location_map.keys()))
        if current_location not in location_map:
            self.location_filter.set("Alle")

        type_values = ["Alle"] + sorted(
            {row["fahrzeug_typ_name"] or row["typ"] or "" for row in rows if row}
        )
        if not type_values:
            type_values = ["Alle"]
        ...
        category_values = ["Alle"] + sorted(
            {row["fahrzeug_kategorie_name"] or row["kategorie"] or "" for row in rows if row}
        )
        if not category_values:
            category_values = ["Alle"]
...
class MasterDataView(ttkb.Frame):
    def __init__(
        ...
        *,
        ...
        allow_user_admin: bool = True,
        current_user: Optional[User] = None,
    ) -> None:
        super().__init__(master)
        self.db = db
        self.current_user = current_user
        self._can_manage_users = allow_user_admin
        ...
        if show_users:
            self.users_frame = UsersFrame(self.content, db, can_manage_users=allow_user_admin)
...
class UsersFrame(ttkb.Frame):
    def __init__(self, master: tk.Misc, db: DatabaseManager, *, can_manage_users: bool = True) -> None:
        super().__init__(master)
        self.db = db
        ...
        toolbar = ttkb.Frame(self)
        toolbar.pack(fill=tk.X, padx=10, pady=10)
        self.add_button = ttkb.Button(toolbar, text="Neu", command=self.add_user, bootstyle="success")
        self.add_button.pack(side=LEFT)
        self.edit_button = ttkb.Button(toolbar, text="Bearbeiten", command=self.edit_user, bootstyle="secondary")
        self.edit_button.pack(side=LEFT, padx=5)
        self.reset_button = ttkb.Button(toolbar, text="Passwort setzen", command=self.reset_password, bootstyle="info")
        self.reset_button.pack(side=LEFT, padx=5)
        self.delete_button = ttkb.Button(toolbar, text="Löschen", command=self.delete_user, bootstyle="danger")
        ...
        if not self._can_manage_users:
            ...
            ttkb.Label(
                self,
                text="Keine Berechtigung für die Benutzerverwaltung.",
                bootstyle="secondary",
    ...
    def _ensure_admin(self) -> bool:
        if self._can_manage_users:
            return True
        Messagebox.show_info("Diese Aktion ist nur für Administratoren verfügbar.", "Keine Berechtigung")
        return False
...
class ComponentsTab(ttkb.Frame):
    def __init__(
        ...
        *,
        ...
        self.lifecycle_btn = ttkb.Button(
            self.toolbar,
            text="Lebenslauf",
            command=self.export_lifecycle,
            bootstyle="info",
        )
        self.lifecycle_btn.pack(side=LEFT, padx=(5, 0))
...
class MedizinprodukteApp(ttkb.Window):
    ...
    def create_widgets(self) -> None:
        ...
        can_manage_users = self._is_admin_user()
        self.master_view = MasterDataView(
            self.view_container,
            self.db,
            show_locations=show_locations,
            allow_edit_locations=allow_edit_locations,
            show_users=can_manage_users,
            allow_user_admin=can_manage_users,
    ...
    def _is_admin_user(self) -> bool:
        return bool(self.user and self.user.role and self.user.role.lower() == "admin")
